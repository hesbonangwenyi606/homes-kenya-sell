...
cat > ~/project-site/homes-kenya-sell/src/components/Header.tsx << 'ENDOFFILE'
import React, { useState, useEffect, useRef } from 'react';
import { Menu, X, Heart, User, ChevronDown, Home, Building, MapPin, LogOut, FileText, Sun, Moon } from 'lucide-react';
import { User as SupabaseUser } from '@supabase/supabase-js';
...
const Header: React.FC<HeaderProps> = ({ favoritesCount, onShowFavorites, onShowAuth, onShowInquiries, user, onSignOut }) => {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [activeDropdown, setActiveDropdown] = useState<string | null>(null);
  const [showUserMenu, setShowUserMenu] = useState(false);
  const [darkMode, setDarkMode] = useState(true);
  ...
  useEffect(() => {
    const savedTheme = localStorage.getItem('theme');
    if (savedTheme === 'light') { document.documentElement.classList.remove('dark'); setDarkMode(false); }
  ...
  const getUserInitials = () => {
    const name = getUserDisplayName().split(' ');
    return name.length > 1 ? name[0].charAt(0).toUpperCase() + name[1].charAt(0).toUpperCase() : name[0].charAt(0).toUpperCase();
  };

  return (
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/90 dark:bg-gray-900/90 backdrop-blur-md shadow-lg border-b border-gray-200/70 dark:border-gray-700/70' : 'bg-white dark:bg-gray-900 shadow-sm'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20 min-w-0">
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-xl flex items-center justify-center flex-shrink-0">
            ...
            <div className="min-w-0">
              <h1 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white truncate">
                Hemaprin<span className="text-emerald-600">Homes</span>
              </h1>
              <p className="hidden sm:block text-xs text-gray-500 dark:text-gray-300 -mt-1">Find Your Dream Home</p>
            </div>
          </div>
          ...
          <nav ref={desktopNavRef} className="hidden lg:flex items-center gap-8">
            <a href="#home" className="text-gray-700 dark:text-gray-200 hover:text-emerald-600 dark:hover:text-emerald-400 font-medium transition-colors">Home</a>
            <Dropdown title="Properties" items={propertyTypes} activeDropdown={activeDropdown} setActiveDropdown={setActiveDropdown} type="icon" />
            <Dropdown title="Locations" items={locationsList} activeDropdown={activeDropdown} setActiveDropdown={setActiveDropdown} type="" />
            <a href="#contact" className="text-gray-700 dark:text-gray-200 hover:text-emerald-600 dark:hover:text-emerald-400 font-medium transition-colors">Contact</a>
          </nav>
          ...
          <div className="flex items-center gap-1 sm:gap-4 flex-shrink-0">
            <button onClick={onShowFavorites} className="relative p-2 text-gray-600 dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
              <Heart className="w-6 h-6" />
              {favoritesCount > 0 && (
                <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-xs rounded-full flex items-center justify-center font-semibold">{favoritesCount}</span>
              )}
            </button>
            <button onClick={toggleTheme} className="flex items-center gap-1 px-2.5 sm:px-3 py-2 rounded-xl bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 hover:scale-105 transition">
              {darkMode ? <Moon size={16} /> : <Sun size={16} />}
...
const Dropdown = ({ title, items, activeDropdown, setActiveDropdown, type }) => (
  <div className="relative">
    <button type="button" onClick={() => setActiveDropdown(activeDropdown === title ? null : title)} className="flex items-center gap-1 text-gray-700 dark:text-gray-200 hover:text-emerald-600 dark:hover:text-emerald-400 font-medium transition-colors">
      {title} <ChevronDown className="w-4 h-4" />
    </button>
    {activeDropdown === title && (
      <div className="absolute top-full left-0 mt-1 w-56 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 py-2 z-20">
        {items.map((item) => {
          const Icon = type === 'icon' ? item.icon : null;
          const key = item.name ?? item;
          return (
            <a key={key} href="#properties" onClick={() => setActiveDropdown(null)} className="flex items-center gap-3 px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-emerald-50 dark:hover:bg-emerald-900 hover:text-emerald-600 transition-colors">
              {Icon && <Icon className="w-4 h-4" />}
              {item.name ?? item}
            </a>
          );
...
const UserMenu = ({ user, getUserDisplayName, getUserInitials, showUserMenu, setShowUserMenu, favoritesCount, onShowFavorites, onShowInquiries, onSignOut }) => (
  <div className="relative">
    <button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
      <div className="w-8 h-8 bg-emerald-600 rounded-full flex items-center justify-center text-white font-semibold text-sm">{getUserInitials()}</div>
      <span className="hidden sm:block text-sm font-medium text-gray-700 dark:text-gray-200 max-w-[100px] truncate">{getUserDisplayName()}</span>
      <ChevronDown className="hidden sm:block w-4 h-4 text-gray-500 dark:text-gray-300" />
    </button>
    {showUserMenu && (
      <>
...
const UserMenuButton = ({ icon: Icon, label, onClick, badge = null, textColor = 'text-gray-700 dark:text-gray-200', hoverColor = 'hover:bg-emerald-50 dark:hover:bg-emerald-900' }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-2.5 ${textColor} ${hoverColor} transition-colors`}>
    <Icon className="w-4 h-4" />
    {label}
...
const MobileMenu = ({ propertyTypes, locations, user, favoritesCount, onShowFavorites, onShowInquiries, onShowAuth, onSignOut, closeMenu }) => {
        ...
        {isOpen && (
          <div className="flex flex-col pl-6">
            {items.map((item) => {
              ...
              const key = item.name ?? item;
              return (
                <a key={key} href="#properties" onClick={closeMenu} className="px-4 py-2 text-gray-700 dark:text-gray-200 hover:bg-emerald-50 dark:hover:bg-emerald-900 rounded-lg flex items-center gap-2">
                  {Icon && <Icon className="w-4 h-4" />} {item.name ?? item}
                </a>
              );
            })}
  ...
  return (
    <div className="lg:hidden border-t border-gray-100 dark:border-gray-700 py-4 max-h-[80vh] overflow-y-auto">
      <nav className="flex flex-col gap-2">
        <a href="#home" onClick={closeMenu} className="px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-emerald-50 dark:hover:bg-emerald-900 rounded-lg font-medium">Home</a>
        <Accordion title="Properties" items={propertyTypes} type="icon" />
        <Accordion title="Locations" items={locations} type="" />
        <a href="#contact" onClick={closeMenu} className="px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-emerald-50 dark:hover:bg-emerald-900 rounded-lg font-medium">Contact</a>
        {user ? (
          <>
            <button onClick={() => { onShowFavorites(); closeMenu(); }} className="px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-emerald-50 dark:hover:bg-emerald-900 rounded-lg flex items-center gap-2"><Heart className="w-4 h-4" /> Saved Properties</button>
