...
const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-white dark:bg-gray-900 shadow-sm sticky top-0 z-50 w-full">
      <div className="w-full px-2 sm:px-4 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={toggleTheme}
              className="flex items-center gap-1 px-3 py-2 rounded-xl bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 hover:scale-105 transition"
            >
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              to={
                title === "Properties"
                  ? `/properties/${(item.name ?? "").toLowerCase()}`
                  : `/locations/${(item.name ?? item).toLowerCase()}`
              }
    ...
    return (
      <div className="flex flex-col w-full">
        <button
          ...
          className="px-4 py-3 w-full text-left flex justify-between items-center text-gray-700 dark:text-gray-200 hover:bg-emerald-50 dark:hover:bg-emerald-900 rounded-lg font-medium"
        >
  ...
  return (
    <div className="lg:hidden w-full border-t border-gray-100 dark:border-gray-700 py-4 animate-slideDownFade max-h-[80vh] overflow-y-auto">
      <nav className="flex flex-col w-full gap-2">
        <Link to="/" onClick={closeMenu} className="px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-emerald-50 dark:hover:bg-emerald-900 rounded-lg font-medium w-full">
          Home
        </Link>
        <Accordion title="Properties" items={propertyTypes} type="icon" />
        <Accordion title="Locations" items={locationsList} />
        <Link to="/contact" onClick={closeMenu} className="px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-emerald-50 dark:hover:bg-emerald-900 rounded-lg font-medium w-full">
          Contact
        </Link>
      </nav>
      ...
      {user ? (
        <div className="mt-2 px-4 flex flex-col gap-2">
          <button onClick={() => { onShowFavorites(); closeMenu(); }} className="px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-emerald-50 dark:hover:bg-emerald-900 rounded-lg flex items-center gap-2 w-full">
            <Heart className="w-4 h-4" /> Saved Properties
          </button>
          <button onClick={() => { onShowInquiries(); closeMenu(); }} className="px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-emerald-50 dark:hover:bg-emerald-900 rounded-lg flex items-center gap-2 w-full">
            <FileText className="w-4 h-4" /> My Inquiries
          </button>
          <button onClick={() => { onSignOut(); closeMenu(); }} className="px-4 py-3 bg-red-50 text-red-600 rounded-xl flex justify-center items-center gap-2 w-full">
            <LogOut className="w-4 h-4" /> Sign Out
          </button>
        </div>
      ) : (
        <button onClick={() => { onShowAuth(); closeMenu(); }} className="px-4 py-3 bg-emerald-600 text-white rounded-xl flex justify-center items-center gap-2 w-full mt-2">
